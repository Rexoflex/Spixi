...
// Source: https://github.com/curiosity-ai/rocksdb-sharp @ f1cf0ba0306fa01b55efa2292c1cc44ee3192b88 (csharp/src/AutoNativeImport.cs), BSD-2-Clause (LICENSE
...
// THIS IS THE ONE EDITED FILE: the Catalyst patch, two edits marked "#920" (Auto.Import and
// GetRuntimeId); the rest is identical to upstream (modulo BOM, CRLF, final newline). Re-vendor: see README.md beside this folder.
#nullable disable
/*
    ...
    public static class Auto
    {
        ...
        public static T Import<T>(string name, string version, bool suppressUnload = false) where T : class
        {
            if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
                ...
                return Importers.Import<T>(Importers.Posix, name, version, suppressUnload);
            /* #920 (Session AC): MAC CATALYST IS POSIX. `IsOSPlatform(OSPlatform.OSX)` is FALSE on
             * Catalyst (its platform is "maccatalyst"), so upstream fell through to the WINDOWS
             * importer and probed `rocksdb.dll` through kernel32 — the July 2026 boot crash
             * (docs/mac-bringup-log.md). dlopen + a Catalyst-stamped dylib is the whole fix. */
    ...
    public static class Importers
    {
        ...
        private static string GetRuntimeId(Architecture processArchitecture)
        {
            ...
            var os =
                ...
                OperatingSystem.IsMacCatalyst() ? "osx" :   // #920: the search path, not the loader (see Auto.Import)
