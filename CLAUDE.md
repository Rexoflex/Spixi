...
- [ ] **Next:**
  1. Batch 3: reactions (+ inside-bubble vs overlap DECISION with Damir — both on screen) · reply rendering (§8-gated) · typing pill · context-menu wiring (spec §5b, sheet-based) · scroll-to-latest button
  2. Figma mirror queue grows: batch-2 card tokens/verification + earlier queue (CLAUDE #70 list)
  3. Then: desktop split-view demo (bubbles exist now — three panes + rail + divider, #19 pitch)
