# Handoff — 2026-08-18 (post-N4, cloud session). READ FIRST.
...
Damir commits the N4 batch (#378–#379; title + body in
`docs/f5-checklist-2026-08-18-n4.md`) on top of `646ef4e9` — that commit should
be at HEAD when you read this; VERIFY. Parent = `646ef4e9`, grandparent
`5fa908b6` (the #375–#377 batch).
...
1. **Extract the tarball, review the diff, commit ONE batch** — steps + commit
   message + F5 legs in `docs/f5-checklist-2026-08-18-n4.md`.
2. **★ Run the N57 protocol** (`docs/n57-triage-group-visibility.md` §2) —
   unchanged, still the top action. Then send the BE engineer be-cutover
   `[Q1-ESC]` + `[NT1]` + the N57 verdict row.
3. **N60 observation on THIS Android deploy:** update over the existing install
   WITHOUT uninstalling; watch for `INSTALL_FAILED_UPDATE_INCOMPATIBLE`.
4. **NEW dial (#378): the launch language set.** Proposal = the 13 live + tr,
...
5. **Native review of the 5 machine drafts** — when you have reviewers, hand
   them `docs/n4-review-notes.md` + `src/strings/draft/<code>.json`.
6. **Dials still open (log answers, no build until given):** N56 wash values ·
...
## 3. NEXT SESSION — the §C fix-first block

`docs/next-session-prompt.md` is the paste-ready prompt. Order: **N13 top
(data-loss class) · N40 · N10 · N15 · N39**; N61 rides the R4 restore round.
Then the rounds R3 → R4 → R7 → R6 → R8 → R5 → R9 per worklist §F. N57 stays
Core-side — do not build; wait for the protocol verdict.
