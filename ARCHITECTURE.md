...
| **Chat** | `onChatScreenReady`/`onChatScreenLoaded` · `setChatMode(type, cost, costText, admin, desc, notif)` · channel functions (`addChannelToSelector`, `setSelectedChannel`, `setChannelSelectorStatus`) · message renderers: `addMe`/`addThem` (11 args) · `addPaymentRequest` (14) · `addFile` (13) · `addAppRequest` (13) · `addCall` (7 — +outgoing/missed/durationSecs, #208) · `updateMessage` · `updateFile` · `updateTransactionStatus` · `updatePaymentRequestStatus` · `addReactions` (+ trailing own-keys arg, #208) · `deleteMessage` · `clearMessages` · `clearInput` · `showUserTyping` · `setNickname` · `setOnlineStatus` · `setUnreadIndicator` · `showWarning` · `showCallButton` · `showRequestSentModal` · `showContactRequest` · `clearApps`/`addApp` · `hideBackButton` · **#523 money pass:** `setCaps(list)` (now carries `tipResult,composeSend,composeRequest,payRequest`) · `setSendQuote(addr, amount, fee, balance)` · `signSendResult(status, txidOrMsg)` · `payRequestResult(msgIdHex, status, msg)` |
