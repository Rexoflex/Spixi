...
|---|---|---|---|
| M1 | Reply to chat bubble | FE BUILT + cap-gated (#79/#25: quote, composer strip, menu Reply) | **GATE — BE-verify-first** (#215 lesson): BE names the carrier + 2-device F5 round-trip/persist BEFORE un-gating. **★ 2026-08-19, asked whether this rides the R3 art round — NO, and the reason is worth keeping:** there is no art and no code left. The whole surface shipped behind a capability flag; un-gating is a one-line cap flip. What is missing is an ANSWER, and the cheapest way to get it is a **standalone carrier-verification step** — an hour, no build: read whether a reply reference survives `StreamMessage` round-trip and persistence in Ixian-Core, then a 2-device F5 to prove it. If the carrier exists, a large user-visible feature lands for one line. If it does not, it goes to the BE cutover with a precise ask instead of "BE claims zero-C#". ⚠ #232 says verify the carrier ON DEVICE first — the #215 lesson (C8 looked 🟢 in the store and was disproven on hardware) applies exactly here |
