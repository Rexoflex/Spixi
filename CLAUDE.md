...
- [ ] **Next:**
  1. **Pattern premium treatment (Damir: "specific color theme based", white isn't premium).** LEAD: `mask-image` with a **data-URI** SVG is CORS-clean even on file:// — inline the pattern as a data-URI mask in CSS (small build step or one-time inline), then ink it with per-theme tokens (`--chat-pattern-ink` returns). Options ranked: data-URI mask > per-theme colored SVG exports > http-only mask. Damir picks the ink color(s)
  2. Batch 3: reactions (+ inside-bubble vs overlap DECISION with Damir — both on screen, #65 now correctly 🟡) · reply rendering (§8-gated) · typing pill · context-menu wiring (spec §5b, sheet-based) · scroll-to-latest button · fold in nearby r2 backlog (typed-card gutter alignment C8, read-vs-delivered color-only C9)
  3. Figma mirror queue grows: batch-2 cards + earlier queue (#70 list) + **r2 deltas: --icon-bubble-read dark = success-800 · sender-label light L = 28%** (⚠️ Damir eyeball the dark-green read checks first)
  4. Then: desktop split-view demo (bubbles exist now — three panes + rail + divider, #19 pitch)
  2. Then batch 2: typed bubbles (payment-request 11305:6344 · payment-card · file-bubble+gap states · app-card · call-card) → batch 3: reactions (+conflict decision) · reply rendering (§8-gated) · typing pill · context menu wiring · scroll-to-latest
  3. Damir eyeballs Figma: chip rows, Overlays section (position/polish to taste — reference components, layout liberties welcome)
  2. `user-circle-filled` icon: Damir makes it manually → account nav selected-state twin automatic
  3. Desktop decisions parked for the split-view demo: empty-state right pane when no chat selected · bottomnav → LEFT RAIL at ≥700px (WhatsApp-style; same component, vertical CSS — cheap, Damir prefers it)
  4. Then: **message bubbles** (Chat&Composite designs exist — scan first per #31) → desktop split-view demo (three panes + draggable divider + left rail + empty state; pitch for #19)
