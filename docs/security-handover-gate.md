...
| item | verb / key / sink / fetch / log? | verdict |
|---|---|---|
| `spixi.chat.glass` (#889) | **ONE new `spixi.*` localStorage key**, boolean (`'1'` or absent), read by chat.html's pre-paint script and mapped to a `data-chat-glass` attribute; nothing else reads it, nothing writes it yet (no toggle). Same class as `spixi.chat.pattern` (#236): no user data, no address, no text. It lives on the shared `file://` partition like every other chat pref (MAJOR #4's partition question is unchanged by it). **★ RETIRED — Session AA (#891, walk row Z.5):** Damir rejected the glass dial, so the key and all four of its readers are deleted. It never shipped a toggle, so nothing ever wrote it and no device holds a value; the `file://` partition surface returns to exactly what it was before #889. Nothing to migrate, nothing to clear. | **introduced, then REMOVED** — no residual surface |
...
## Session AC (#917–#920) — the flag probe deferred, the landscape round, the iOS push extension, route B (2026-09-22)

One question per row: does this exposure exist at the baseline?

| item | verb / key / sink / fetch / log? | verdict |
|---|---|---|
| `Spixi-PushService/SpixiPushGate.cs` + `NotificationService.cs` (#919) | **A NEW PROCESS parses attacker-influenced input.** A push's `fa` (the sender address, set by the IPN server from the stream message) now reaches OUR code in the extension process before the SDK renders anything: read from `custom.a.fa` or a flat `fa`, compared against a set, used as `threadIdentifier` and as a dictionary key for the title. It is used as an opaque string only (no eval, no URL, no file path); the trace line prints `fa=none|present`, never the value. Same class as Android's `SpixiNotificationServiceExtension` (#512: REACH INTRODUCED) — that row's verdict carries over: **ours, by design of #503/#915**, and the parsing surface is the string-equality + dictionary lookup shown, nothing else. | **INTRODUCED — accepted (mirrors the Android row)** |
| `Platforms/iOS/SPushPrefsShare.cs` → `spixi-push.json` in the App Group container (#919) | **A NEW STORAGE LOCATION holding contact addresses and nicknames** (the muted set + the nick map, the latter only when `senderName`) — unencrypted JSON, readable by the app and the extension (both signed with the same team, that is what an App Group is). Written only while the node is running (a pre-unlock write would EMPTY it, not leak it); excluded from iCloud/Finder backup; cleared on `wipeEverything`. The baseline had no App Group and no such file; Android's equivalent is the extension reading `Preferences` in-process. The exposure = another copy of data the app already stores in plaintext (CORE-11 covers chat storage at rest; the contact list is not encrypted today either). | **INTRODUCED — accepted; the at-rest question is CORE-11's, not a new class** |
| OneSignal native code in the extension process (`OneSignalSDK.DotNet` 6.1.9 in `Spixi-PushService.csproj`; `OneSignal_app_groups_key` in both Info.plist) (#919) | third-party code now runs in a second process with App Group access. The key lets the SDK write confirmed-delivery receipts to the same container. Baseline: the SDK already runs in the app with the same trust. | **INTRODUCED — accepted; same vendor, same trust tier** |
| `installFlagFontLater()` (#917) | scheduling only — the SAME install as #888, later; no new key, sink, fetch or log | none |
| `src/shells/home.html` landscape media block · `settings-screens.css` · `overlay.css` / `wallet-receive.css` sheet cap (#918) | CSS only | none |
| `Platforms/MacCatalyst/RocksDbSharp/**` (#920) | **43 vendored files of third-party C# compiled for ONE TFM** (maccatalyst) — a dynamic-library LOADER (`dlopen` by name from the app's own bundle paths + the `osx` runtimes folder, `Reflection.Emit` for the P/Invoke stubs). Baseline: the same wrapper's compiled DLL ships in the public package on Windows; the dev's DLL on iOS/Android. The one edit is a platform-detection branch. Integrity: `MANIFEST.sha256` recomputed by the suite. ⚠ The loader searches `AppContext.BaseDirectory` and the assembly directory first — a writable app bundle is not a threat on a signed Catalyst app, and the same search order already runs on Windows. | **INTRODUCED — accepted (same code, one more platform); the dylib itself is machine-local and gitignored** |
| `cs-syntax-check.mjs` · `.gitattributes` · `.gitignore` · the sheets under `docs/sheets/session-ac/` | dev machine only | none |
| `setInsetSides` push + `*SL{AndroidInsetLeft/Right}` carriers + `*SL{SpixiPlatform}` (#922) | **one new bridge PUSH** (C# → every generated document, two numeric args, regex-validated `^\d{1,3}(\.\d{1,2})?$` before they touch a custom property — the AND-45 grammar) and **three new carriers** (two seeded numbers, one compile-time platform name — no user data, no address). `data-platform` is read only by `attachLandscapeRail` for a layout convention; the landscape decision reads `screen.width/height` + `screen.orientation.type` (device geometry the document already exposes — no new surface, no fetch, no storage). The side-inset pushes also dispose a PARKED Account overlay (#315) so a stale inset cannot re-present — a teardown, not a new reach. | none (mirrors the AND-45 row) |

Session AC adds no `spixi.*` key, no verb, no HTML sink and no network fetch (one new PUSH, `setInsetSides`, numeric-validated like `setInsetBottom`; #923 reads `DeviceDisplay.MainDisplayInfo` — device geometry, no user data — nothing to file). It adds ONE storage location outside the app's sandbox (the App Group file) and ONE new process that reads a push payload — both accepted above with their reasons. The introduced-vs-inherited census gains the three #919 rows and the #920 row and is otherwise unchanged. ⚠ Neither the extension nor the vendored wrapper has been compiled anywhere yet; the rows describe the source.

## Session AD (#928–#930) — the ours/his cutover rows: eleven verbs and pushes, one purge, four keys retired (2026-09-23)

One question per row: does this exposure exist at the baseline? Loop on Opus, three rounds (#929).

| item | verb / key / sink / fetch / log? | verdict |
|---|---|---|
| `ixian:landtab:<id>` on SettingsPage → `HomePage.landOnTab` → `landOnTab` push (S11) | **A new verb + a new push, and a KEY RETIRED.** The id is matched against the three tab names (`tab1`/`tab2`/`tab3`) before anything moves; an unknown id is dropped. Replaces the `spixi.landtab` localStorage handshake (an address-free tab name, but a `spixi.*` key on the shared `file://` partition) and its `[LANDTAB]` probe. Cancel-first (#797). | INTRODUCED, reach smaller than what it replaces — a key left the partition |
| `spixi.settings.view` stash retired (#274 row) | **A key RETIRED.** No writer, no reader; the boot restore it fed cannot fire since #334 (no reload survives). | none — a removal |
| `spixi.exdel.*` retired + boot sweep (C16) | **A key FAMILY RETIRED** (the #254 ruling's own hint — shape `{del,t,kind}`, no text). `home.html` sweeps stale `spixi.exdel.` keys once at boot and writes none. C16's premise was refuted (Core persists the remote delete); the fix is `UIHelpers.refreshChatRow` on both local-delete paths — a push, no new reach. | none — a removal |
| `spixi.backup.last` retired → `setLastBackup` push (S2) | **A key RETIRED, a push ADDED.** C# owns the stamp (`Preferences` `lastBackupTimestamp`, a number); the push carries the number or `""`. The nudge is gated in C# (`HomePage.displayBackupReminder`), so the WebView no longer decides when to nag. ⚠ dial (#930): the stamp is written after the share sheet RETURNS, and a cancel returns too — it records "the sheet was opened". | INTRODUCED (a push carrying a timestamp), a key left the partition |
| `ixian:favchat:<addr>:on\|off` + `setChatFavorite` echo; `Preferences` `fav.<addr>` (CH4) | **A new verb, a new push, a NEW PREFERENCE FAMILY keyed by ADDRESS** (app storage — `Preferences`, not the WebView partition; the baseline stored nothing per contact there beyond mute/notify, which this mirrors). The verb's address is validated with the same `Address` parse every other HomePage verb uses before it touches a preference; the warn line on a malformed token prints the TYPE, never the token (⚠ the PRE-EXISTING `mutechat` twin prints it — inherited, filed in the security-review NITs). Cleared on every app-side removal path; ⚠ NOT on Core-side removals (#930) — a stale `fav.<addr>` is a boolean under an address the app no longer holds, no content. | INTRODUCED (a preference under an address) — accepted: mirrors the existing mute family |
| `ixian:dev` on SettingsPage + cap `dev` + `setCapDev` push (S9) | **A verb that opens `DevPage`** (send-log / share / HUD — the same page the 10-tap HomePage toggle already opens), gated on the persisted `devMode` at the C# handler (the cap only draws the row; the verb refuses without the preference). Presented by the in-stack HomePage (an overlay page's own modal never shows). | none new — a second entry to an existing surface, same gate |
| `ixian:undorequest` on ContactDetails (C17/CO1) + `setRelation`/`addContact` +2 — ★ **the verb was DELETED by Session AF #947; `setRelation`/`addContact` stay** | **A verb that REMOVES a contact record** (`FriendList.removeFriend`) — guarded by exactly the relation's "pending" set (never an approved contact, never a group, never a bot, never an incoming request), the answer pushed on both arms INSIDE the try (the gate-5 axis walk reads it there). SingleChatPage/HomePage already carry the same verb (#562/B1). | none new — a third entry, stricter guard |
| the `:media` suffix on `removehistory`/`removecontact`/`leavegroup` → `SContacts.collectReceivedMedia` + `purgeFiles` (CH3) | **★ A NEW FILESYSTEM DELETE PATH.** No WebView-supplied string reaches it: the suffix is a boolean, the paths come from `FriendMessage.filePath` (written by C# at `TransferManager.completeFileTransfer`), each re-checked by `isInsideDownloadsRoot` (fail-closed: empty · relative — `IsPathRooted` first · escaping · throwing → false) before `File.Delete`. RECEIVED + completed rows only. The other-contact walk spares shared paths and runs off-thread. ★ r3 finding, FIXED: the leaf is the peer's name while S16 is open — a recorded path with any `.`/`..` segment never re-roots (`hasDotSegment` on the RAW string). Residual (#930): an orphaned file (owning history gone) is not protected; deletion stays inside the Downloads root. Own raw history parser — never `getMessages`, which re-arms outgoing transfers with open handles. | INTRODUCED — accepted with the three guards; one user action ("delete media") behind it |
| `requestFundsResponse` store mutation + write outside the chat-page gate (W11) | **A money-adjacent WRITE moved out of a UI gate.** Nothing signed, nothing broadcast: the store records the txid the PAYER sent. ★ r1 MAJOR fixed: the row lookup was peer-writable by id → the payer side's own four-clause rule (id · `requestFunds` · `localSender` · unanswered), the txid parsed (`txIdLegacyToV8`) BEFORE storing, a miss BREAKS before any push; the predicate is pinned as a whole, canonical string. | INTRODUCED (a write that used to be dropped) — FIXED in-loop, pinned |
| `addPaymentRequest` +4 args (C1/C2) · `addPaymentActivity` +2 (CI2) · `addCall` +1 (C4) · `addReactions` +1 (C6) · `addChatReaction` +1 (CH8) · `addChat` +1 (CH6) · `showUserTyping(who, nick)` (C21) · `onRequestResult`/`onValidAddress` (CO3/CO4) · `showUrlError(reason)` (A4) | **Pushes WIDENED, one push per row.** Every new arg is a C#-computed enum, number, boolean or address the receiving shell already held in another form; the shells degrade on an old exe's arity (pinned). `insufficient` is a DISPLAY hint — the native review page decides. CH8's reactor and C21's typist are suppressed under `hidesParticipants` (blind rooms), as the baseline's roster is. | none new — the same data, typed |
| `MiniAppManager.fetchWithReason` (A4) | **The existing app fetch under ONE 15 s deadline** (was 30 s end to end); no new host, no new URL source — the same user-typed URL. The reason pushed to the shell is a fixed vocabulary (`fetchFailureName`: url · http · size · timeout · invalid · error), never the URL. | none new |
...
One new trailing `addChat` argument: the sender's display name for a group or bot-room tail (a nick the recipient already sees on the bubble, resolved the same way as `SingleChatPage.resolveNick`), or the address when nothing resolves. **Lens:** no verb, no `spixi.*` key, no WebView setting, no network fetch. The value is peer-controlled, so it is rendered with `textContent` only (`chatlist-item.js createExcerpt`) — pinned on the built shell with a markup nick (smoke #944). An address is shortened by the shell (#211). The one log line added (`resolveExcerptSender`) records the exception TYPE only, no nick and no address. ~~**Verdict: nothing introduced.**~~ **CORRECTED by the Session AF review (#946, E-1): the verdict was WRONG.** In a BLIND room the fallback returned the member's address — on the owner's device the member's REAL address — where the bubble shows "Hidden member" and never an address. That exposure is INTRODUCED (the baseline list showed no sender). **Fixed** in the same session: `resolveExcerptSender` returns "" under `Utils.hidesParticipants(friend)` (fails closed, as CH8's reactor push does), pinned by an ordered guard+return walk and killed by mutation. Blind BOT rooms are not masked by the bubble either (#613) — parity, not a leak.

## Session AF (#946–#951, 2026-09-24) — the AE walk fixes

| row | change | lens | verdict |
|---|---|---|---|
| #946 | `resolveExcerptSender` blind guard | closes the #944 exposure above | FIXED (introduced → removed) |
| #947 | ContactDetails `ixian:undorequest` branch DELETED; the pending profile renders no action | removes a WebView-reachable `removeFriend` path; no verb, key, sink or fetch added | nothing introduced (surface REDUCED) |
| #948 | `.c-member__actions` moved to member-sheet.css | CSS only | nothing introduced |
| #949 | in-card hairlines removed (Account hub, member roster) | CSS only | nothing introduced |
| #950 | `hasOwnTip` pre-check before any tip transaction is prepared or confirmed; new lang key | money path: REFUSES earlier, signs nothing new; the log line names no address or amount | nothing introduced |
