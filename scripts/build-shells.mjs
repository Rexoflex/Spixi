...
process.on('exit', () => {
  ...
  if (checkDiffs.length) {
    ...
    console.log('build-shells --check: every generated artifact matches a fresh build \u2713 (images/ are verbatim copies — not compared; Sora is EMBEDDED in the compared css; fonts/TwemojiCountryFlags.js — the flag font as a data: URL — is regenerated and compared)');
  }
});
...
/* ★ L15b (Session Z): the flags-only Twemoji face ships BESIDE the shells as a runtime asset
 * — never inlined — and AS A SCRIPT, not as a font file: the shells load from file:// on
 * every platform and a @font-face url() fetch is a CORS request Chromium refuses from a
 * file: document (WebView2 has no opt-in switch in this app; #46 loop auditor B-1). A classic
 * <script src> is not CORS-checked and a data: URL is CORS-exempt, so the woff2 travels as
 * `window.__spixiFlagFont = 'data:font/woff2;base64,…'` in fonts/TwemojiCountryFlags.js and
 * flags.js injects that script only on a device that cannot paint a flag (Windows). Sora
 * stays EMBEDDED in the CSS (the inliner's job); this is the ONE file fonts/ holds, and the
 * suite pins that. --check regenerates and compares it like every other artifact. */
const FLAG_FONT_SRC = join(root, 'src', 'assets', 'fonts', 'TwemojiCountryFlags.woff2');
if (!existsSync(FLAG_FONT_SRC)) throw new Error('build-shells: src/assets/fonts/TwemojiCountryFlags.woff2 is missing — the shells call installFlagFont() and would find no payload on Windows');
const FLAG_FONT_OUT = join(OUT_DIR, 'fonts', 'TwemojiCountryFlags.js');
{
  const b64 = readFileSync(FLAG_FONT_SRC).toString('base64');
  const payload = '/* L15b (Session Z): the flags-only Twemoji subset (CC-BY 4.0, docs/legal/third-party-notices.md) as a data: URL — loaded by flags.js installFlagFont() only on a device that cannot paint a flag emoji; a script because a file:// font fetch is CORS-refused. Generated by build-shells from src/assets/fonts/TwemojiCountryFlags.woff2 — do not edit. */\n'
    + "window.__spixiFlagFont = 'data:font/woff2;base64," + b64 + "';\n";
  if (!CHECK) mkdirSync(join(OUT_DIR, 'fonts'), { recursive: true });
  writeFileSync(FLAG_FONT_OUT, payload);   // the CHECK-aware writer: compares under --check, writes + NUL/short-write-gates otherwise
  console.log('  · ' + (CHECK ? 'checked' : 'generated') + ' fonts/TwemojiCountryFlags.js (' + (payload.length / 1024).toFixed(0) + ' KB, the runtime flag font as a data: URL, L15b)\n');
}
