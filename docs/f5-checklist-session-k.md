...
```
A `backstop t=` line means the verb did not arrive in 400 ms — say so, it is the #663 class (the walk K capture hit a 150 ms first cut once: the renderer ran the queue ~100 ms after C#'s drain marker; 400 now).
