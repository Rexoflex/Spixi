...
|---|---|
| **Home** | `selectTab` · `loadAvatar` · `setVersion` · `setAddress` · `setHideBalance` · `selectChat` / `selectTx` (split view) · `clearContacts`/`addContact(addr, nick, avatar, online, unread, relation, kind)` (+2, Session AD C17/CO1 — `relation` ∈ none/contact/pending-in/pending (+ the group/bot arms), `kind`) · `clearChats`/`addChat(addr, nick, ts, avatar, online, excerpt, type, unread, kind, mention, excerptKind, excerptSender)` (+1, Session AD CH6 — the excerpt KIND; the shell's locale-carrier reverse map is gone · +1, #944 — the tail's SENDER for a group/bot row, "" otherwise; an address-shaped value is shortened by the shell)/`clearChatsDone` · `addChatReaction(addr, nick, key, unixSecs, reactor)` (+1, Session AD CH8 — the reactor's address, never under `hidesParticipants`) · `setChatFavorite(addr, "1"|"0")` (Session AD CH4 echo of `ixian:favchat:`) · `landOnTab(id)` (Session AD S11 — replaces the `spixi.landtab` handshake) · `showUrlError(reason)` (Session AD A4 — the HomePage add-app host; `AppNewPage` pushes the same) · `setUnreadIndicator` · `clearPaymentActivity(filter)`/`addPaymentActivity(txid, received, counterparty, time, amount, fiat, confirmed)`/`clearPaymentActivityDone` · `setLastBackup(unixSecs|"")` (Session AD S2 — C# owns the stamp; gates the nudge) · `clearApps`/`addApp(...)`/`clearAppsDone` · `setBalance(balance, fiat, nick)` · `setContactStatus` · `showWarning` · `toggleAnimatedSlider` · `showRatingPrompt` · `updateDebugInfo` · **#523 money pass:** `setCaps(list)` · `setSendQuote(addr, amount, fee, balance)` · `signSendResult(status, txidOrMsg)` · `quickScanResult(data)` |
| **Chat** | `onChatScreenReady`/`onChatScreenLoaded` · `setChatMode(type, cost, costText, admin, desc, notif)` · channel functions (`addChannelToSelector`, `setSelectedChannel`, `setChannelSelectorStatus`) · message renderers: `addMe`/`addThem` (11 args) · `addPaymentRequest` (18 — +kind · statusEnum ∈ pending/paid/declined · fiat · insufficient, Session AD C1/C2; `updatePaymentRequestStatus`/`updateTransactionStatus` carry the enum) · `addFile` (13) · `addAppRequest` (13) · `addCall` (9 — +outgoing/missed/durationSecs #208, +callActive Session AD C4) · `updateMessage` · `updateFile` · `updateTransactionStatus` · `updatePaymentRequestStatus` · `addReactions` (+ trailing own-keys arg #208, + tip TOTAL 4th arg Session AD C6; the tip token is `tip:<amount>`) · **Session P (#298 B1+B3, DECISIONS #801):** `addMessages(base64JSON, position)` — the load burst as ONE push: `{strs:[…], items:[{f, a, r?}]}`, each item = one of EIGHT allowlisted names (the six row renderers above + the `showContactRequest` marker + a standalone `addReactions`; `f` names it, `a` = its argument list in the old order — never empty — an integer arg indexes `strs` — interned long `data:` URIs ≥ 256 chars — and `r` = that row's `addReactions` pair folded onto its own row); an item outside the allowlist is dropped and counted (`[chat-shell] addMessages dropped=<n>` at WARN); `position` = `append` (today) | `prepend` (the shell contract for B2, not yet sent) · `messagesDone` — the end-of-batch signal (ends the shell's render burst and paints ONCE; the 250 ms safety timer stays for an old exe). The per-row renderers stay live for live arrivals · `deleteMessage` · `clearMessages` · `clearInput` · `showUserTyping(who, nick)` (Session AD C21 — the typist, non-blind rooms only) · `setNickname` · `setOnlineStatus` · `setUnreadIndicator` · `showWarning` · `showCallButton` · `showRequestSentModal` · `showContactRequest` · `clearApps`/`addApp` · `hideBackButton` · **#523 money pass:** `setCaps(list)` (now carries `tipResult,composeSend,composeRequest,payRequest`) · `setSendQuote(addr, amount, fee, balance)` · `signSendResult(status, txidOrMsg)` · `payRequestResult(msgIdHex, status, msg)` |
| **Contact details** | `setAddress` · `setNickname` · `setAvatar` · `showIndicator` · `setRelation(relation)` (Session AD C17) · ~~`undoRequestResult(addr, status)`~~ (RETIRED Session AF #947 — the pending pane has no Cancel and ContactDetails no longer answers `ixian:undorequest`) · `clearRecentActivity`/`addPaymentActivity(txid, type, time, amount, confirmed, status, direction)` — 7-arg since Session AD CI2 (four-state `status` · `in`/`out`); the old 5-arg exe degrades |
| **Contact new** | `setAddress` · `onValidAddress(address)` (Session AD CO4 — echoes the resolved address) · `onRequestResult("0", reason)` (Session AD CO3 — the three refusals name themselves) |
...
| **Lock** | `setJustConfirm` |
| **Settings** | `setNickname` · `setAppearance` · `setLockEnabled` · `showRemoveAvatar` · `loadAvatar` · `onBack` (hardware-back bounce) · `setVersion` · `setAddress` (plain identity form, #208) · `setLanguage` · `setPaymentAuth(bool)` (PA1 #525) · `setLastBackup(unixSecs|"")` (Session AD S2; re-pushed on a warm re-present via `onRepresentedNative`) · `setCapDev("1"|"0")` (Session AD S9 — the Developer row under the persisted `devMode`) |
