...
consumed here). Read this whole file, then **DECISIONS #789–#795**, then — before touching the
...
```
bundle 321 · shells 18 · smoke BASELINE OK 4125 / the 3 known (#136 · M5 · B3)  ← WITH the sibling
...
**4090 → 4125.** No new export (321), no new string (786), no new C# file. **Nine
...
⚠ **THE MSBUILD WIRING IS UNVERIFIED HERE** (no toolchain). Auditor A caught its MAJOR before
...
① **The capture** (the phone): the two new `[CDPERF]` lines × 3 opens (checklist §2B) + the
   one still owed since #788 (`present <page> by=paint|timer`, one temporary line in
   `presentPreload`). **Nothing in the perf work order is believed until B1 exists.**
② **Damir's rulings**: D1 doodle SVGO land/keep · D2 lossy PNGs · D3 contacts-es.svg ·
   D4 the four Session M dials.
③ **The pre-warm build** per `docs/prewarm-chat-spec.md` — with the phone in the room, the
   chats-list frame probe first, a #46 loop after. ⛔ #779 stays parked.
